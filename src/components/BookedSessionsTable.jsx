...
export default function BookedSessionsTable({ initialBookings }) {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      const { data: tokenData } = await authClient.token();
      const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/bookings/${cancelId}`, {
        method: "PATCH",
        headers: { 
  ...
  if (bookings.length === 0) {
    return (
      <div className="text-center py-16 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-dashed border-gray-200 dark:border-gray-800">
        <p className="text-gray-500 dark:text-gray-400 font-semibold text-lg">You haven't booked any learning sessions yet.</p>
        <Button onClick={() => router.push("/tutors")} className="mt-4 bg-cyan-500 text-white font-bold rounded-lg px-6">
          Explore Tutors
        </Button>
      </div>
  ...
  return (
    <>
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[700px]">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 text-gray-700 dark:text-gray-200 text-sm font-bold">
              <th className="p-4">Tutor Name</th>
              <th className="p-4">Student Name</th>
              <th className="p-4">Email</th>
              <th className="p-4">Status</th>
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-800 text-sm text-gray-600 dark:text-gray-300 font-medium">
            {bookings.map((booking) => (
              <tr key={booking._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                <td className="p-4 font-bold text-gray-800 dark:text-white">{booking.tutorName}</td>
                <td className="p-4">{booking.studentName}</td>
                <td className="p-4">{booking.studentEmail}</td>
                <td className="p-4">
                  <span className={`px-2.5 py-1 rounded-md text-xs font-extrabold ${
                ...
                <td className="p-4 text-center">
                  {booking.bookStatus !== "Cancelled" ? (
                    <button
                      onClick={() => setCancelId(booking._id)}
                      className="px-3 py-1.5 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 rounded-lg font-bold text-xs hover:bg-red-500 hover:text-white transition-all"
                    >
                      Cancel
                    </button>
                  ) : (
                    <span className="text-xs text-gray-400 dark:text-gray-500 font-semibold italic">No actions available</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      {cancelId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <Card className="bg-white dark:bg-gray-900 max-w-sm w-full p-6 relative rounded-xl shadow-2xl border border-gray-100 dark:border-gray-800 text-center">
            <h3 className="font-black text-xl text-gray-800 dark:text-white mb-2">Cancel Session?</h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">Are you sure you want to cancel this booked tutoring session appointment?</p>
            <div className="flex justify-center gap-3">
              <Button type="button" onClick={() => setCancelId(null)} variant="flat" className="rounded-lg font-bold">No, Keep It</Button>
              <Button onClick={handleCancelBooking} disabled={loading} className="bg-red-500 text-white font-bold rounded-lg px-6 hover:bg-red-600">
                {loading ? "Cancelling..." : "Yes, Cancel"}
